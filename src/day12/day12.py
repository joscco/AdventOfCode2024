with open("input.txt") as f:
    ...
    def is_in_grid(i, j):
        return 0 <= i < len(values[0]) and 0 <= j < len(values)
    ...
    def get_neighbors_of_same_value(i, j, value) -> list:
        ...
        for neighborIndex in get_neighborIndices(i, j):
            if is_in_grid(*neighborIndex) and values[neighborIndex[1]][neighborIndex[0]] == value:
    ...
    connected_components = get_connected_components(values)
    ...
    print(sum([get_product(component) for component in connected_components]))
    ...
    def get_number_of_sides(component):
        ...
        for n in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            for edge in sorted({(s[0] + n[0], s[1] + n[1]) for s in seen} - seen, key=lambda p: (p[0], p[1])):
                exists = next((side for side in sides for p, d in side if abs(p[0] - edge[0]) + abs(p[1] - edge[1]) == 1 and d == n), None)
    ...
    print(sum([get_product_2(component) for component in connected_components]))
